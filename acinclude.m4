...
dnl is deduced from the value of JAVAH. When this macro completes, a
...
dnl
dnl - at the configure.in level, set JAVAH=yourcompiler before calling
dnl AC_JNI_INCLUDE_DIR
dnl
dnl - at the configure level, setenv JAVAH
dnl
...
JNI_INCLUDE_DIRS=""
test "x$JAVAH" = x && AC_MSG_ERROR(['\$JAVAH' undefined])
AC_PATH_PROG(_ACJNI_JAVAH, javah, no)
test "x$_ACJNI_JAVAH" = xno && AC_MSG_ERROR([$JAVAH could not be found in path])

_ACJNI_FOLLOW_SYMLINKS("$_ACJNI_JAVAH")
